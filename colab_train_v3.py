#!/usr/bin/env python3
"""ZigZag ML Predictor - V3 (Fixed Labels & Classes)"""
...
print("="*70)
print("ZigZag ML Predictor - V3 (Fixed Labels)")
print("="*70)
...
# STEP 3: Better ZigZag Labels - Track actual HH/HL/LH/LL patterns
print("\n[3/7] Creating BETTER ZigZag labels...")
labels_list = []
...
# Generate labels using simple logic
for i in range(30, len(df) - 5):
    try:
        # Look at last 3 significant moves
        lookback = 15
        window = df.iloc[max(0, i-lookback):i+1]
        
        # Find swing highs and lows in window
        swing_high = window['high'].max()
        swing_low = window['low'].min()
        swing_high_idx = window['high'].idxmax()
        swing_low_idx = window['low'].idxmin()
        
        # Find previous swing (use bars before current window)
        ...
        # Classify based on swing high/low positions
        # HH: current high > previous high, current low > previous low (uptrend)
        # HL: current high < previous high, current low > previous low (correction in uptrend)
        # LH: current high < previous high, current low < previous low (downtrend)
        # LL: current high > previous high, current low < previous low (bounce in downtrend)
...
# STEP 6: Evaluate - CRITICAL: Ensure all classes are present in predictions
...
for name, m in models.items():
    try:
        ...
        # CRITICAL: Always ensure 5 classes
        if proba.shape[1] < 5:
        ...
        assert proba.shape[0] == len(X_test), f"{name}: Length mismatch {proba.shape[0]} vs {len(X_test)}"
        assert proba.shape[1] == 5, f"{name}: Classes mismatch {proba.shape[1]} vs 5"
        proba_list.append(proba)
    except Exception as e:
        print(f"    Error with {name}: {e}")
        # Fallback: create uniform proba
...
if proba_list:
    ...
    if test_accuracy >= 0.80:
        ...
        print("\n⚠️  Consider fine-tuning hyperparameters")
    
    print("\nNext Steps:")
    print("  1. Save models to pickle")
    print("  2. Upload to Hugging Face Hub")
    print("  3. Deploy as API endpoint")
