#!/usr/bin/env python3
"""ZigZag MT4 Final Correct Implementation

Key fix: Labels HH/HL/LH/LL must compare with:
- For High: with last_high
- For Low: with last_low

Not just with the previous extremum!
"""
...
class ZigZagMT4Final:
    """Final correct MT4 ZigZag implementation"""
    ...
    def label_zigzag(self, extrema):
        ...
        CORRECT LOGIC:
        - Track both last_high and last_low separately
        - For High: compare with last_high → HH if higher, HL if lower
        - For Low: compare with last_low → LL if lower, LH if higher
        """
        if len(extrema) < 2:
            return extrema
        ...
        # First extremum - no label, but initialize tracking
        first_type = extrema[0][2]
        first_price = extrema[0][1]
        
        if first_type == 'H':
            last_high = first_price
        else:
            last_low = first_price
        ...
        for i in range(1, len(extrema)):
            ...
            if curr_type == 'H':
                ...
                if last_high is None:
                    # First High (shouldn't happen after init)
                    label = None
                else:
                    ...
                    if curr_price > last_high:
                        ...
                        label = 'HL'  # Higher Low (current High is lower than last High)
                
                # Update last_high
                last_high = curr_price
            
            else:  # curr_type == 'L'
                # This is a Low
                if last_low is None:
                    # First Low (shouldn't happen after init)
                    label = None
                else:
                    # Compare with last low
                    if curr_price < last_low:
                        label = 'LL'  # Lower Low
                    else:
                        label = 'LH'  # Lower High (current Low is higher than last Low)
...
print("="*80)
print("ZigZag MT4 Final Correct Implementation")
print("="*80)
...
for depth in [8, 12, 15]:
    for deviation in [3, 5, 7]:
        zz = ZigZagMT4Final(depth=depth, deviation=deviation, backstep=2)
...
zz = ZigZagMT4Final(depth=depth_choice, deviation=deviation_choice, backstep=backstep_choice)
