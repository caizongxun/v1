#!/usr/bin/env python3
"""ZigZag ML Predictor - Working Version"""
...
# Calculate metrics
test_accuracy = accuracy_score(y_test_eval, ensemble_pred)
...
print(classification_report(y_test_eval, ensemble_pred, target_names=label_names, zero_division=0))

cm = confusion_matrix(y_test_eval, ensemble_pred, labels=list(range(5)))
print(f"\nConfusion Matrix:")
print(cm)
...
for i in range(5):
    mask = y_test_eval == i
    if mask.sum() > 0:
        class_acc.append(accuracy_score(y_test_eval[mask], ensemble_pred[mask]))
    else:
        class_acc.append(0)
